...
export default defineComponent({
  ...
  setup(props, { slots, attrs, emit, expose }) {
    const { prefixCls, autoInsertSpaceInButton, direction, size } =
    ...
    const classes = computed(() => {
      ...
      return {
        ...
        [`${pre}-two-chinese-chars`]:
          hasTwoCNChar.value && autoInsertSpace.value,
        [`${pre}-block`]: block,
    ...
    return () => {
      ...
      isNeedInserted =
        children.length === 1 && !icon && !isUnborderedButtonType(props.type);

      const { type, htmlType, disabled, href, title, target, onMousedown } =
        props;
